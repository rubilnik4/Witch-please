package tarot.domain.entities

import shared.models.tarot.authorize.Role
import tarot.domain.models.projects.{Project, ProjectId}
import tarot.domain.models.users.{User, UserId, UserProject}

import java.util.UUID

final case class UserProjectEntity(
  userId: UUID,
  projectId: UUID,
  role: Role
)

object UserProjectEntity {
  def toDomain(userProjectEntity: UserProjectEntity): UserProject =
    UserProject(
      userId = UserId(userProjectEntity.userId),
      projectId = ProjectId(userProjectEntity.projectId),
      role = userProjectEntity.role
    )

  def toEntity(userProject: UserProject): UserProjectEntity =
    UserProjectEntity(
      userId = userProject.userId.id,
      projectId = userProject.projectId.id,
      role = userProject.role
    )
}
