...
import shared.models.tarot.spreads.SpreadStatus.Published
import sttp.tapir.server.ziohttp.ZioHttpInterpreter
import tarot.api.endpoints.*
import tarot.domain.models.TarotError
import tarot.domain.models.spreads.{SpreadId, SpreadStatusUpdate}
  ...
  override def spec: Spec[TestEnvironment & Scope, Any] = suite("Spread publish API integration")(
    test("initialize test state") {
      for {
        ...
        spreadId <- TarotTestFixtures.getSpread(projectId, photoId)
