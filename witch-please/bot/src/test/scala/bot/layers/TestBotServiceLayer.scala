...
import shared.infrastructure.services.telegram.{TelegramChannelService, TelegramChannelServiceLayer, TelegramWebhookLayer}
import zio.ZLayer

object TestBotServiceLayer {
  val botServiceLive: ZLayer[BotConfig, Throwable, BotService] =
    (
      (BotServiceLayer.telegramConfigLayer >>> TelegramChannelServiceLayer.telegramChannelServiceLive) ++
